...
local function OnAuraDataChanged(unitToken)
	...
	-- same unitToken (e.g. duel starts -> UnitCanAttack becomes true), the cached
	-- container references may be nil for the now-active options. Rebuild lazily.
...
local function OnNamePlateAdded(unitToken)
	...
	-- BUGFIX (duels / faction flips): Previously this returned early if no containers
	-- were created for the current options table (e.g. friendly player with Friendly.*
	-- all disabled). That meant `nameplateAnchors[unitToken]` and `watchers[unitToken]`
	-- were never populated, so when the unit later flipped faction (duel start) and
	-- GetUnitOptions() started returning Enemy options, there was no watcher listening
	-- to UNIT_AURA and OnAuraDataChanged would never fire to rebuild containers.
	-- We now always create data+watcher as long as at least one side (current OR
	-- opposite faction) has any mode enabled; OnAuraDataChanged will lazily build
	-- the correct containers when the active faction's options become "Enabled".
	...
	if not ccContainer and not importantContainer and not combinedContainer and not anyEnabledOpposite then
		-- Neither current nor opposite faction has any mode enabled, so there is truly
		-- nothing to watch for this unit. Keep the original fast-path.
		return
	end
	-- If only the opposite faction has any mode enabled, we still keep a watcher alive
	-- so that a future faction flip (duel start) can materialize containers.
...
function M:Init()
	...
	-- BUGFIX (instance transitions): IsModuleEnabled() depends on IsInInstance()/IsInRaid(),
	-- which change at runtime (entering/leaving arenas, battlegrounds, dungeons, raids).
	-- Without these events, the module cannot detect when its activation state transitions,
	-- forcing users to manually toggle the Enabled checkbox to get buff/CC icons to appear.
	-- Note: HaveModesChanged() only tracks *config* changes, not *context* changes, so on a
	-- context transition we must force RebuildContainers() ourselves.
	eventFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
	eventFrame:RegisterEvent("ZONE_CHANGED_NEW_AREA")
	-- BUGFIX (duels / faction flips): A friendly player who accepts a duel, or any unit whose
	-- faction/PvP flags change, transitions between friendly<->enemy WITHOUT the nameplate
	-- being removed/re-added. GetUnitOptions() then returns a different options table
	-- (Enemy vs Friendly), but the cached container references in `nameplateAnchors` still
	-- reflect the previous faction's Enabled setup (often nil if only one side was enabled),
	-- so OnAuraDataChanged silently drops aura data because `data.CcContainer == nil`.
	-- Re-running OnNamePlateAdded rebuilds the containers using the new faction's options.
	eventFrame:RegisterEvent("UNIT_FACTION")
	eventFrame:RegisterEvent("PLAYER_FLAGS_CHANGED")

	local previousModuleActive = moduleUtil:IsModuleEnabled(moduleName.Nameplates) and AnyEnabled()
	...
	eventFrame:SetScript("OnEvent", function(_, event, unitToken)
		...
		elseif event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then
			-- Re-evaluate activation state; if it transitioned from disabled -> enabled
			-- (e.g. entering an arena/BG/dungeon), rebuild containers so existing
			-- nameplates get their watchers attached. M:Refresh() alone is not enough
			-- because HaveModesChanged() only tracks config changes.
			local nowActive = moduleUtil:IsModuleEnabled(moduleName.Nameplates) and AnyEnabled()
			if nowActive and not previousModuleActive then
				ApplyBlizzardNameplateSettings()
				EnableWatchers()
				RebuildContainers()
			elseif not nowActive and previousModuleActive then
				DisableWatchers()
			end
			previousModuleActive = nowActive
		elseif event == "UNIT_FACTION" then
			-- Only react if the unit currently has a visible nameplate tracked by us.
			-- unitToken here is the unit whose faction changed (e.g. "player", "target",
			-- or a nameplate/party/raid token). We need to find the matching nameplate
			-- unitToken and re-run OnNamePlateAdded so containers are rebuilt against
			-- the new faction's options.
			if unitToken and nameplateAnchors[unitToken] then
				OnNamePlateAdded(unitToken)
				OnAuraDataChanged(unitToken)
			else
				-- Fallback: UNIT_FACTION may fire with a non-nameplate token (e.g. "player"
				-- when you enter a duel). Walk all tracked nameplates and rebuild any
				-- whose faction classification has changed.
				for token in pairs(nameplateAnchors) do
					OnNamePlateAdded(token)
					OnAuraDataChanged(token)
				end
			end
		elseif event == "PLAYER_FLAGS_CHANGED" then
			-- Duel start/end and PvP flag toggles can affect UnitIsEnemy() for the
			-- affected unit. Rebuild containers for any tracked nameplate that matches.
			if unitToken and nameplateAnchors[unitToken] then
				OnNamePlateAdded(unitToken)
				OnAuraDataChanged(unitToken)
			end
